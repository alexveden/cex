#pragma once
#include <include/cex.h>
#include "allocators.h"

/**
 * @brief Dynamic array (list) implementation
 */
typedef struct
{
    // NOTE: do not assign this pointer to local variables, is can become dangling after dlist
    // operations (e.g. after realloc()). So local pointers can be pointing to invalid area!
    void* arr;
    size_t count;
} dlist_c;

#define dlist$define(eltype)                                                                       \
    /* NOTE: shadow struct the same as dlist_c, only used for type safety. const  prevents user to \
     * overwrite struct arr.arr pointer (elements are ok), and also arr.count */                   \
    struct __CEX_TMPNAME(__cex_dlist__)                                                            \
    {                                                                                              \
        eltype* const arr;                                                                         \
        const size_t count;                                                                        \
    }

#define dlist$new(self, capacity, allocator)                                                       \
    (dlist.create(                                                                                 \
        (dlist_c*)self,                                                                            \
        capacity,                                                                                  \
        sizeof(typeof(*(((self))->arr))),                                                          \
        alignof(typeof(*(((self))->arr))),                                                         \
        allocator                                                                                  \
    ))

typedef struct
{
    struct
    {
        size_t magic : 16;
        size_t elsize : 16;
        size_t elalign : 8;
    } header;
    size_t count;
    size_t capacity;
    const Allocator_c* allocator;
    // NOTE: we must use packed struct, because elements of the list my have absolutely different
    // alignment, so the dlist_head_s takes place at (void*)1st_el - sizeof(dlist_head_s)
} __attribute__((packed)) dlist_head_s;

_Static_assert(sizeof(dlist_head_s) == 32, "size");
_Static_assert(alignof(dlist_head_s) == 1, "align");

struct __module__dlist
{
    // Autogenerated by CEX
    // clang-format off

Exception
(*create)(dlist_c* self, size_t capacity, size_t elsize, size_t elalign, const Allocator_c* allocator);

Exception
(*append)(void* self, void* item);

Exception
(*extend)(void* self, void* items, size_t nitems);

size_t
(*count)(void* self);

void*
(*destroy)(void* self);

void*
(*iter)(void* self, cex_iterator_s* iterator);

    // clang-format on
};
extern const struct __module__dlist dlist; // CEX Autogen
