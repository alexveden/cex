...
#define _jw$buf_var _json_buf_macro_scope

/// Opens JSON buffer scope (json_buf_ptr data is cleared out)
#define jw$buf(json_buf_ptr, jsontype_arr_or_obj)                                                  \
    _cex_json__buf__clear((json_buf_ptr));                                                         \
    for (json_buf_c * _jw$buf_var __attribute__((__cleanup__(_cex__jsonbuf_print_scope_exit))) =   \
...
#define jw$kobj_scope(key)                                                                         \
    _cex_json__buf__print(_jw$buf_var, "\"%s\": ", key);                                           \
    jw$obj_scope()
...
#define jw$fmt(format, ...) _cex_json__buf__print(_jw$buf_var, format, __VA_ARGS__)

/// Append string item into array scope (jw$buf)
#define jw$str(format, ...) _cex_json__buf__print_item(_jw$buf_var, "\"" format "\"", __VA_ARGS__)

/// Append value item into array scope (jw$buf)
#define jw$val(format, ...) _cex_json__buf__print_item(_jw$buf_var, format "", __VA_ARGS__)

/// Append `"<key>": "<format>"` (with your own format) into object scope (jw$buf)
#define jw$kstr(key, format, ...)                                                                  \
    _cex_json__buf__print_key(_jw$buf_var, (key), "\"" format "\"", __VA_ARGS__)

/// Append `"<key>": <format>` into object scope (jw$buf)
#define jw$kval(key, format, ...) _cex_json__buf__print_key(_jw$buf_var, (key), format, __VA_ARGS__)
