...
CexSerdeGen_process_file(CexSerdeGen_c* self, char* path)
{
    ...
    mem$arena(256 * 1024, _)
    {
        ...
        if (!code){
            return e$raise(Error.io, "Error reading file: %s", path);
        }
        ...
        while ((t = CexParser.next_entity(&lx, &items)).type) {
            ...
            if (d == NULL) { continue; }
            e$ret(CexSerdeGen.process_decl(self, &lx, d));
        }
    }
...
_CexSerdeGen_codegen_serialize_field(CexSerdeGen_c* self, cex_codegen_s* cg$var, serdegen_field_s* f)
{
    ...
    if (field_type) {
        // Project Type
        cg$scope ("e$except_silent (err, %s.%s.serialize(jw, item->%s, json_writer_kwargs)) {",
                  self->namespace,
                  field_type->name,
                  f->name) {
            cg$pn("jw->error = err;");
...
_CexSerdeGen_generate_type(CexSerdeGen_c* self, cex_codegen_s* cg$var, serdegen_type_s* t)
{
    ...
    cg$func ("Exception %s__%s__serialize(jw_c* jw, %s* item, jw_kw* json_writer_kwargs) ",
             self->namespace,
             t->name,
             t->name) {
        cg$if ("!item") {
    ...
    cg$func ("Exc %s__%s__print(%s* item, jw_kw* json_writer_kwargs) ",
             ...
             t->name) {
        cg$pn("jw_c jw;");
        cg$pn("e$ret(jw$new(&jw, .stream = stdout, .indent = 0));");
        cg$pf("return %s.%s.serialize(&jw, item, NULL);", self->namespace, t->name);
    }
...
Exception
CexSerdeGen_process_decl(CexSerdeGen_c* self, CexParser_c* lx, cex_decl_s* d)
{
    uassert(self);
    uassert(lx);
    uassert(d);
...
Exception
CexSerdeGen_process(CexSerdeGen_c* self)
{
...
const struct __cex_namespace__CexSerdeGen CexSerdeGen = {
    ...
    .process = CexSerdeGen_process,
    .process_decl = CexSerdeGen_process_decl,
